...
def _trend_sentence(res: pd.DataFrame) -> str:
    """Direction of the recent trend from a linear fit over monthly counts."""
    ...
    txt = (f" Monthly reporting shows {_b(direction)} across the period, with the "
           f"latest month at {_b(f'{int(last):,}')} incidents "
    ...
    if abs(z).max() >= 2:
        ...
        txt += (f" {_b(peak_month)} stands out as a statistical outlier "
                f"({int(monthly.loc[peak_month]):,} incidents) and merits closer review.")
    return txt
...
def _severity_sentence(res: pd.DataFrame) -> str:
    ...
    return (f" By severity, {_b(worst)} carries the heaviest toll at "
            f"{_b(f'{worst_ratio:.1f}')} victims per incident, against an "
            f"overall average of {_b(f'{overall:.1f}')}.")
