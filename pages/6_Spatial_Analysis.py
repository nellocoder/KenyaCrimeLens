...
page_header("🗺️", "Spatial Analysis",
            "Bubble size shows incidents per county; colour intensity shows the victim toll")
...
kpi_cards([
    ...
    {"icon": "👥", "label": "Mapped victims", "value": f"{int(mapped['Victim Tally'].sum()):,}",
     "sub": "media-reported"},
...
with col1:
    map_metric = st.radio(
        "Map shows bubble size & colour by:",
        ["incidents", "victims"],
        horizontal=True,
        index=0,
        key="map_metric"
...
st.caption(
    ...
    "Bubble size and colour follow the selected metric."
)